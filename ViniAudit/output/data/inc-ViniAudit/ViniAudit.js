...
function hideAll() {
    ...
    $("[id*='.details']").hide()
    var element = document.getElementById('Vini_display_account_id_on_all_pages')
...
function updateNavbar(path) {
    ...
    if (path === '') {
        $('#ViniAudit_navbar').addClass('active')
...
/**
 * Get the format of the results that Vini Audit is reading from
 */
function getFormat() {
...
function loadMetadata() {
    if (getFormat() === resultFormats.json) {
        runResults = getViniAuditResultsJson()
    } else if (getFormat() === resultFormats.sqlite) {
...
/**
 * Show About Vini Audit modal
 */
function showAbout() {
    $('#modal-container').html(about_ViniAudit_template())
    $('#modal-container').modal()
}
