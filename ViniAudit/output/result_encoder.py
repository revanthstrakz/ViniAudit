...
from ViniAudit import DEFAULT_REPORT_DIRECTORY
from ViniAudit.core.console import print_exception, print_info
from ViniAudit.output.utils import get_filename, prompt_for_overwrite


class ViniJsonEncoder(json.JSONEncoder):
    """
    JSON encoder class
    """
...
class ViniResultEncoder:
    ...
    @staticmethod
    def to_dict(config):
        return json.loads(json.dumps(config, separators=(',', ': '), cls=ViniJsonEncoder))


class SqlLiteEncoder(ViniResultEncoder):
...
class JavaScriptEncoder(ViniResultEncoder):
    """
