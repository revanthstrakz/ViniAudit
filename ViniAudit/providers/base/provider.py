...
from ViniAudit import __version__ as Vini_version
from ViniAudit.core.console import print_exception, print_info, print_error
from ViniAudit.output.html import ViniReport
from ViniAudit.providers.base.configs.browser import get_object_at
...
class BaseProvider:
    ...
    Root object that holds all of the necessary provider-specific resources and Vini configuration items.
    ...
    async def fetch(self, regions=None, excluded_regions=None, partition_name=None):
        ...
        """
        This is quite ugly but the legacy Vini expects the configurations to be dictionaries.
        Eventually this should be moved to objects/attributes, but that will require significant re-write.
        """
        report = ViniReport(self.provider_code, 'placeholder')
    ...
    def manage_object(self, object, attr, init, callback=None):
        """
        This is a quick-fix copy of Opinel's manage_dictionary in order to support the new ViniAudit object which isn't
        a dict
        """
        if type(object) == dict:
            ...
            if 'summaries' in self.metadata[service_group]:
                for summary in self.metadata[service_group]['summaries']:
                    for callback in self.metadata[service_group]['summaries'][summary]['callbacks']:
                        ...
                        # quick fix as legacy Vini expects "self" to be a dict
                        target_object = self
